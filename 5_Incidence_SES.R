...
### Use imputed data
HIV_edu_SES$SES <- HIV_edu_SES$wealth_quant_pca.imp1 

### Aggregate data for SES

# Summarise the data
aggregated_data_SES <- HIV_edu_SES %>%
  group_by(Round_Year, SES) %>%
  summarise(
...
aggregated_data_SES <- aggregated_data_SES %>%
  mutate(
    total_PY = round(total_PY, 2),
    incidence = round(incidence, 2)
  )
# Drop rows with NA values for SES
aggregated_data_SES <- dplyr::filter(aggregated_data_SES, !is.na(SES))
# Create flextable
flextab <- flextable(aggregated_data_SES) %>%
...
# Convert Round_Year to numeric
aggregated_data_SES$Round_Year <- as.numeric(as.character(aggregated_data_SES$Round_Year))



# Recode SES as a factor with meaningful labels
aggregated_data_SES$SES <- factor(aggregated_data_SES$SES,
                                  levels = c("1","2","3"),
...
SES_plot <- ggplot(aggregated_data_SES, aes(x = Round_Year, y = incidence, color = SES, fill = SES)) +
  ...
  labs(
    title = paste0("Incidence by Round Year and SES with 95% confidence intervals 
...
doc <- read_docx() %>%
  ...
  body_add_par("Incidence by SES and Year", style = "heading 1") %>%
  ...
  body_add_gg(value = SES_plot, width = 10, height = 5, style = "centered") %>%
  body_end_block_section(value = landscape_one_column)

print(doc, target = paste0(output_dir,"/inc_SES_",as.character(age_min),"-"
