### Code to look at changes in association between education and incidence over time 
...
start_year = 2005
end_year = 2023
#n_fact = 3 # Number of quantiles in edu
...
for (i in 1:sim_num) {
print(i)
mdat[[i]]  <- merge(mdat[[i]] ,Vis_edu,by.x=c('IIntID','Year'),by.y=c('IIntId','Visit_Year'),all.x=TRUE)
...
AggByEduYear = AggFunc("highest_edu + Year")
...
sformula = "sero_event ~ -1 +    as.factor(Year):as.factor(highest_edu) + as.factor(Year) + as.factor(highest_edu)   + offset(log(tscale))"
...
newdata <- unique(tmp_m_1.df[c('Year','highest_edu')])
newdata <- dplyr::filter(newdata,!is.na(newdata$highest_edu))
newdata <- newdata %>% 
  dplyr::arrange(Year,highest_edu)
newdata$year_edu <- paste0(as.character(newdata$Year),"-",as.character(newdata$highest_edu))
newdata <- newdata[c('year_edu','Year','highest_edu')]
newdata$year_ses <- as.factor(newdata$year_edu)
newdata$Year <- as.factor(newdata$Year)
newdata$wealth_quantile <- as.factor(newdata$highest_edu)
newdata$tscale <- 1
...
inc_plot_data[c('Year', 'Edu')] <- stringr::str_split_fixed(inc_plot_data$terms, '-', 2)
inc_plot_data <- inc_plot_data[c('Year','Edu','fit','lci','uci')]
...
plot_title <- paste0("Smoothed Incidence stratified by Highest Education ")


p1 <- ggplot(data=Edu_inc_plot, aes(x=Year, y=Incidence, group=Edu,
                                    color=Edu,ymin=lci, ymax=uci)) +
  geom_line() +
  geom_errorbar( width=.2) +
  theme_classic() +
  theme_igray() + scale_colour_colorblind() +
  scale_x_continuous(name ="Year",breaks = seq(2009,2021, by = 1),limits = c(2009, 2021)) +
  scale_y_continuous(name ="Incidence",breaks = seq(0,10, by = 1),limits = c(0,10)) +
  labs(color='Edu',
       title = plot_title) 
...
g1 <- ggplot(data=Edu_inc_plot) + 
  stat_smooth(aes(x=Year, y = Incidence, colour = Edu), method = "loess", se = FALSE) +
  stat_smooth(aes(x=Year, y = uci, colour = Edu), method = "loess", se = FALSE) +
  stat_smooth(aes(x=Year, y = lci, colour = Edu), method = "loess", se = FALSE)
#g1
...
gg1_dat <- data.frame(Year = gg1$data[[1]]$x,
                  ...
                  lci_sm = gg1$data[[2]]$y,
                  uci_sm = gg1$data[[3]]$y,
                  Edu = as.factor(gg1$data[[1]]$group))

gg1_dat$Highest_Education <- as.character(gg1_dat$Edu)
...
p3 <- ggplot() + 
   ...
   #                                  group=Highest_Education, fill=Highest_Education),alpha=0.2) +
   geom_smooth(data=gg1_dat,aes(x=Year, y=Incidence,group=Highest_Education,colour=Highest_Education), se = FALSE, linetype = "dashed" ) +
